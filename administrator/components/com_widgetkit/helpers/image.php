...
class WidgetkitImage {
	...
	*/
    protected function _options($options) {
		...
		// set size
		if (($options['width'] == 'auto' || !$options['width']) && ($options['height'] == 'auto' || !$options['height'])) {
			$options['width'] = $source['width'];
			$options['height'] = $source['height'];
		} else if (($options['width'] == 'auto' || !$options['width']) && is_numeric($options['height'])) {
			$options['width'] = @($options['height'] / $source['height']) * $source['width'];
		} else if (($options['height'] == 'auto' || !$options['height']) && is_numeric($options['width'])) {
