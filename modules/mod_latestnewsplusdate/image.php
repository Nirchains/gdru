...
class lnpResizeImageHelper
{
	public static function getResizedImage($image, $newWidth, $newHeight, $option='crop', $quality='90')
	{
		...
		$optionArray = lnpResizeImageHelper::getDimensions($newWidth, $newHeight, $width, $height, $option);
		$optimalWidth  = $optionArray['optimalWidth'];
		$optimalHeight = $optionArray['optimalHeight'];
		//Resample - create image canvas of x, y size
		$imageResized = imagecreatetruecolor($optimalWidth, $optimalHeight);
		if(($extension=='.png')||($extension=='.gif')){
			lnpResizeImageHelper::setTransparency($imageResized,$img); 
		}
		...
		switch($extension)
		{
			...
			case '.jpeg':
				...
				$sharpness = lnpResizeImageHelper::findSharp($width, $optimalWidth);
				$sharpenMatrix = array(
					array(-1, -2, -1),
	...
	private static function findSharp($orig, $final) // function from Ryan Rud (http://adryrun.com)
	{
