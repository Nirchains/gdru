...
class plgSystemWidgetkit_Zoo extends JPlugin {
	...
	public function onAfterInitialise() {
        ...
        // check for update file, remove after update
        if (JFile::exists($update = dirname(__FILE__).'/update.php')) {
            include($update);
            JFile::delete($update);
        }
	...
	public function initTypeLayouts($event) {
		...
		foreach ($this->widgetkit['path']->dirs("zoowidgets:") as $widget) {
			if ($this->widgetkit['path']->path("zoowidgets:$widget/renderer")) {
				$name = JText::_('Widget') . ' ' . ucfirst($widget);
				$extensions[$name] = array('type' => 'modules', 'name' => $name, 'path' => $this->widgetkit['path']->path("zoowidgets:$widget"));
			}
		}
